...
lapply(c("tidyverse", "ggplot2", "ggrepel", "sf", "WDI", "regions", "tidytext", "gutenbergr", "readr", "readxl", "showtext", "purrr", "ggridges", "ggdist", "xtable", "ggrepel", "extrafont", "WDI", "ggtext", "ggdist", "forecats", "patchwork"),  pkgTest)
...
ggplot() + 
  geom_sf(data = world_map, 
          aes(fill = as.factor(MAPCOLOR7)), #all have diff category names
...
tragedy_sentiment <- tragedy_words |> 
  inner_join(get_sentiments("bing"), by = join_by(word), relationship = "many-to-many") #inner join = prescent in 
