...
lapply(c("scales", "tidyverse", "ggplot2", "lubridate", "patchwork", "geofacet", "ggrepel", "GGally", "corrgram", "ggcorrplot", "broom", "marginaleffects"),  pkgTest)
...
ggplot(IRE_weather[IRE_weather$station=="Dublin",], aes(x = date, y = maxtp)) +
  geom_line() +
  scale_y_continuous(sec.axis = sec_axis(trans = ~ (. * 9/5) + 32, name = "Fahrenheit")) + #transform to farenheight
...
ggplot(data = eu_gdp |> filter(north_europe == 1), aes(x = year, y = gdp_pc)) +
  ...
  facet_wrap(vars(name), scales = "free_y", nrow = 3) + #theme is voice
  theme_void() +
...
# Marginal = all other predictors as constant
...
table(jr$AREA1) #1 Due Process – criminal and administrative procedure
...
jr$DATE <- dmy(jr$DATE)
jr$S.DATE <- dmy(jr$S.DATE)

jr$DIFF <- jr$S.DATE - jr$DATE
mean(jr$DIFF, na.rm = TRUE)

# What is the most dissenting votes that were cast in the case in which the USSC reversed the decision of the lower court?
jr %>% filter(REVERSD == "1") %>%  summarise(
  max_dissent = max(DISSENT, na.rm = TRUE)
)
  # 7
...
jr <- jr %>% group_by(YEAR) %>% mutate(n_cases = n()) %>% ungroup()
  
total_count <- ggplot(jr, aes(x = YEAR, y = n_cases)) +
  geom_line() +
  geom_smooth() +
  theme_minimal()
...
invalidate_share <- ggplot(jr_invalidate, aes(x = YEAR, y = share_invalid)) +
  geom_line() +
  geom_smooth() +
  theme_minimal()

pdf("patch_jr_plot.pdf")
# library(patchwork)
total_count + invalidate_share
dev.off()
